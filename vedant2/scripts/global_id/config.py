...
class P4AConfig:
    ...
    # C.1: radius (metres) used to count CONFIRMED/LOST tracks near a track at the
    # moment it transitions CONFIRMED -> LOST, captured as density_at_loss.
    local_density_radius_m: float = 5.0
    ...
    # C.1/F2: a track lost while surrounded by a dense grouping (density_at_loss)
    # is more likely to re-enter as an ID-swap into the wrong nearby body, so its
    # appearance-based re-entry vetoes (pose, posture) are tightened by this factor
    # once density_at_loss >= density_gate_tighten_min_count. The positional/
    # kinematic re-entry gates (reentry_mahalanobis_gate, kinematic reachability)
    # are never touched by this — only appearance.
    density_gate_tighten_min_count: int = 3
    density_gate_tighten_factor: float = 0.6
    ...
    def __post_init__(self) -> None:
        for name in ("confirm_hits", "lost_window_frames",
        ...
        for name in ("shadow_confirm_override_hits", "expected_roster_max"):
            if type(getattr(self, name)) is not int or getattr(self, name) <= 0:
                raise ValueError(f"{name} must be a positive integer")
        for name in ("shadow_confirm_gate_m", "roster_cap_min_separation_m", "local_density_radius_m"):
        ...
        if type(self.density_gate_tighten_min_count) is not int or self.density_gate_tighten_min_count < 0:
            raise ValueError("density_gate_tighten_min_count must be a non-negative integer")
        _range("density_gate_tighten_factor", self.density_gate_tighten_factor, 0.0, 1.0)
        ...
        if type(self.confirm_bonus_scale) is not int or self.confirm_bonus_scale <= 0:
            raise ValueError("confirm_bonus_scale must be a positive integer")
        _nonnegative("lost_window_k1", self.lost_window_k1)
        _nonnegative("lost_window_k2", self.lost_window_k2)
