...
| **P3.5** binding 3D lift | Triangulate a full skeleton per binding + check for "chimeras" (two people fused) | per-binding RANSAC-DLT triangulation, per-joint 3D covariance, one-sided reprojection bias names an intruding camera | `lift3d.jsonl`, `lift_purity.json` |
