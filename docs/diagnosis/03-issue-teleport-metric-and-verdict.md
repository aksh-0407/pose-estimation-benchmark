...
1. **Grazing-angle foot noise.** A bbox bottom projected to the ground at a grazing camera
   ...
   the *only* source, and 1 m in 20 ms = 50 m/s → an instant false teleport. This is why
   `M2_2_4_1` scores agreement **0.992** (identity essentially perfect) yet **158 teleports**
   → `fail`. The identity is right; the metric is measuring foot-projection noise.
...
- `M2_2_4_*` group: agreement 0.94–0.99, teleports 121–236 → all `fail`. High agreement is
...
## Fix direction (see ../../wip/to_do.md item A1)
