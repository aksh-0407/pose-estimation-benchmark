...
| **05a** global ID (online) | Turn per-frame clusters into **persistent IDs** that live the whole clip | **Singer-acceleration ground Kalman** per player + staged assignment (binding to exact tracklet to χ²-gated Hungarian to re-entry); emits the **Kalman posterior** ground position | `global_player_id`, `ground_tracks.jsonl` |
