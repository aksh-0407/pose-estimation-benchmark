...
1. **Membership churn in P3.** The tracklet joins cluster X (→ global id A) on some frames and
   cluster Y (→ id B) on others, because the per-frame association is re-decided and the
...
## Fix direction (see ../../wip/to_do.md item A7)
