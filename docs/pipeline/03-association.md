...
inverse-covariance fusion in an A/B on real data (0.176 / 0.248 / **0.145** m — `../diagnosis/README.md` §1a).
...
- **ID-1 (★★★) Cross-camera under-merge on facing pairs.** X-cam agreement **0.50 on _7**,
  0.77–0.80 on _5/_6/M2 (`../diagnosis/09-per-phase-issue-register.md` ID-1). Root: low-parallax facing geometry + dead colour
...
- **ID-5 (★★) Single-linkage cannot split → permanent chimeras.** Cluster cycle-consistency
  **0.68–0.90** (`../diagnosis/README.md` V2-L2): 10–32% of ≥3-view clusters are geometrically
  inconsistent, i.e. two people merged and un-splittable (`../diagnosis/09-per-phase-issue-register.md` ID-5).
- **ID-4 (★★) Appearance cue statistically dead.** d′ ≈ 0.00–0.09 on 5/8 deliveries
  (`../diagnosis/09-per-phase-issue-register.md` ID-4) — near-identical kit + desaturation. The 0.20 colour weight is mostly
...
| 2 | **Give the clustering the ability to split** — replace single-linkage with **correlation clustering / graph multicut** (or an iterative refine-split pass) gated on the 04 (binding lift) full-skeleton reprojection. | ★★★ | Single-linkage's merge-only nature makes ID-5 chimeras permanent; a splittable objective can undo an early wrong merge; the triangulation reprojection is a clean split signal (see [04-lift.md](04-lift.md)). | 10–32% inconsistent clusters resolvable → fewer chimeras + teleports. | High | correlation-clustering / multicut MOT [Tang et al. 2017]; unified multi-view tracking [2302.03820] |
