...
change list (`../roadmap.md`).

Pipeline order: **P1** (2D pose) to **P1b** (2D stabilization) to **P2** (per-camera tracking)
 to  **P3** (cross-camera association) to **P3.5** (binding 3D lift) to **P4a** (online global id)
 to  **P4b** (stitch + colocated merge) to **P5** (roles) to **P6** (terminal 3D) to **R** (render).
...
| P1-3 | jitter_px 1.2-3.4 (panel), 2D keypoint noise is 2-3 px, the floor for reprojection error; hips are the worst joint (11-12 px cross-view). | manager reprojection analysis (`roadmap.md` §H). | 3D reproj |
...
| P4a-3 | **Loose chi² gate (5.991) + Singer process noise** admit a far measurement after a short gap to posterior lurches even with `emit_kalman_posterior`. | e_max 500-1500 on posterior-emit clips. | 04 |

## P4b, stitch (min-cost-flow) + colocated merge (W9)

| # | Issue | Evidence | Feeds |
|---|---|---|---|
| P4b-1 | **Gap-scaled spatial budget** (`distance ≤ v_max·gap·slack`) lets a long occlusion license a cross-field stitch to emitted step. | `stitching.py:171-176`. | 04, 06 |
| P4b-2 | **Mean-over-fragments emission** (`runner.py:348`) averages the positions of everything merged into an id per frame to oscillation when two are concurrent. | 1528 emitted big jumps. | 04 |
| P4b-3 | **Flat 0.75 m colocated-merge radius** misses grazing-offset splits (cam_04 foot 1 m off) to split id survives; 2 residual coloc pairs. | `M1_1_14_7`, `M2_1_11_3`. | 05 |
...
keeper pick (`roadmap.md` §B), needs mosaic sign-off, not a code fix. Peripheral
...
| P6-3 | Hip keypoint cross-view inconsistency to 11-12 px reproj (worst joint). | reproj analysis. | reproj |
...
**P1 deep-field recall gap to single-camera players to (P3) no binding cue on grazing/facing
cameras to split identity + (P4a) over-mint to (P4b) risky stitch + mean-emission to emitted
teleports + flicker; (P6) drops the same frames to coverage gaps.** Fix upstream coverage and
