...
| --- | --- | --- |
| `python -m core.inference.run_phase1_rtmpose_inference` | P1 | RTMDet detect + RTMPose-X pose to 2D keypoints (COCO-17 + Halpe-26). |
...
| `python -m identity.p3_association.run_cross_camera_association` | 03 | Cluster the same player across cameras (tracklet graph to `binding_id`). |
| `python -m identity.p4_lift.run_triangulation` | 04 / terminal | Multi-view 2D to 3D world skeleton (binding-keyed, then global-keyed). |
...
| `python -m identity.export.export_ue_packets` | export | Triangulated 3D to Unreal Engine pose packets. |
...
- **`python -m main`**, full chain P1 to 06 + render across deliveries; `--from-stage`/`--until-stage`
...
- **`python -m identity.id_pipeline`**, the identity-only inner loop (association to global-id over
  a frozen tracking tree) with the metric panel.

Common flags: `--dataset`, `--version`, `--deliveries`, `--output-tree`, `--artifacts-root`
(mosaics to `data/viz/<dataset>/pipetrack_v<num>/`), `--drive-root`, `--jobs`, `--skip-render`,
`--panel-only`, `--baseline`.
