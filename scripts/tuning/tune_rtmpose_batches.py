...
def parse_args() -> argparse.Namespace:
    ...
    tuning.add_argument("--repeats", type=int, default=2,
                        help="Repeat each batch pair this many times (default: 2)")
...
def build_command(args: argparse.Namespace, det_batch: int, pose_batch: int, run_id: str) -> list[str]:
    command = [
        sys.executable,
...
def ranked_summary(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row["status"] != "success" or row["fps"] in (None, ""):
            continue
        key = (int(row["det_batch_size"]), int(row["pose_batch_size"]))
        grouped.setdefault(key, []).append(row)

    summary: list[dict[str, Any]] = []
    for (det_batch, pose_batch), group in grouped.items():
...
def main() -> int:
    ...
    total = len(args.det_batches) * len(args.pose_batches) * args.repeats
    ...
    for det_batch in args.det_batches:
        for pose_batch in args.pose_batches:
            for repeat in range(1, args.repeats + 1):
                counter += 1
                run_id = f"{args.prefix}-db{det_batch}-pb{pose_batch}-r{repeat}"
                command = build_command(args, det_batch, pose_batch, run_id)
                mpath = manifest_path(args, run_id)
                print(f"\n[{counter}/{total}] det={det_batch} pose={pose_batch} repeat={repeat}")
