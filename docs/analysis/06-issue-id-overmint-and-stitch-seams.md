...
## Evidence of over-mint to stitch
...
- **Fragmentation**, a per-camera tracklet that breaks (occlusion, fast sprint, missed
  detection) yields a new cluster to new track; P4b is meant to re-stitch it.
...
- **Good stitch**: same player, short gap, spatially continuous seam to id count drops, no
  jump (e.g. `M1_1_16_4`: 3 merges, 0 big jumps).
- **Bad stitch**: two fragments whose seam spans a big spatial gap (allowed because
  `distance ≤ v_max·gap·slack` grows with gap) to an emitted **step** at the join. Or a
  concurrent-in-disjoint-cameras merge to an emitted **oscillation** (`04-...`).
