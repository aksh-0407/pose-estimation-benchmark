...
- **cam_04 (end-on, down the pitch)** — dominates every top pair (01-04, 02-04, 03-04,
  ...
  - **Grazing ground projection.** End-on, a player's bbox-bottom projects to the ground with
    ~1 m depth error, so the ground-distance cue that P3 fuses is noisy for cam_04 → its
...
- **cam_07 (panoramic 3775×960, oblique)** — players are tiny (10–20 px), pose confidence is
  ...
  frequently fail to associate (`M2_2_3_5`: cam_07 → P016 vs cam_05 → P009). cam_05↔cam_07 is
...
## Consequence chain
split identity → extra distinct ids (over-mint) → the id also appears as the "other" player's
